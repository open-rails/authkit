...
func TestPermissionGroupStore_WalkAndAuthorize(t *testing.T) {
	...
	if len(asg) != 1 || asg[0].GroupType != "org" || len(asg[0].Roles) != 1 || asg[0].Roles[0] != OwnerRoleName {
	...
	// Resource addressing: (type, resource_ref) -> internal id.
	if got, err := st.GroupByResourceRef(ctx, "org", "acme"); err != nil || got != orgID {
		t.Errorf("GroupByResourceRef(org,acme) = %q,%v; want %q", got, err, orgID)
	}
