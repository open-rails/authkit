...
// IntrinsicRootType returns the base `root` GroupTypeDef authkit ships: the
...
func IntrinsicRootType(extraRootRoles ...RoleDef) GroupTypeDef {
	...
	// super-admin == root:* (owner is also seeded = root:* by normalizeType).
	roles = append(roles, RoleDef{Name: SuperAdminRoleName, Permissions: []string{OwnerGrant(RootType)}})
...
// the bare IntrinsicRootType() is injected. This is the consumer entry point:
...
func BuildSchema(appTypes ...GroupTypeDef) (*GroupSchema, error) {
	...
	types := appTypes
	if !hasRoot {
		types = append([]GroupTypeDef{IntrinsicRootType()}, appTypes...)
	}
