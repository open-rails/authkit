...
// role" lifecycle, replacing the old org_invites. An invite names a single role
...
// Group ids stay INTERNAL; callers address the owning group by (type, resource_ref).
...
func (s *Service) AcceptGroupInvite(ctx context.Context, inviteID, userID string) error {
	...
	// group_id + role out so the assignment uses the invite's own attribution.
	var groupID, role, status string
	var expiresAt *time.Time
	err = q.QueryRow(ctx,
		`SELECT group_id::text, role, status, expires_at
		 FROM profiles.group_invites
		 WHERE id = $1::uuid AND user_id = $2::uuid AND deleted_at IS NULL
		 FOR UPDATE`,
		inviteID, userID).Scan(&groupID, &role, &status, &expiresAt)
