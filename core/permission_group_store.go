...
// SeedContainment upserts the containment schema (group_type_parents) from a
...
func (st *PermissionGroupStore) SeedContainment(ctx context.Context, schema *GroupSchema) error {
	for _, typeName := range schema.Types() {
		if schema.IsRoot(typeName) {
			continue
		}
		td, _ := schema.Type(typeName)
		for _, parent := range td.AllowedParents {
			if _, err := st.q.Exec(ctx,
				`INSERT INTO profiles.group_type_parents (type, allowed_parent_type)
				 VALUES ($1, $2) ON CONFLICT DO NOTHING`,
				typeName, parent); err != nil {
				return fmt.Errorf("seed containment %s<-%s: %w", typeName, parent, err)
			}
		}
	}
	return nil
...
// parentType are empty for the root group. The containment trigger + CHECK
...
func (st *PermissionGroupStore) CreateGroup(ctx context.Context, groupType, parentID, parentType, resourceRef string) (string, error) {
	...
	err := st.q.QueryRow(ctx,
		`INSERT INTO profiles.permission_groups (type, parent_id, parent_type, resource_ref)
		 VALUES ($1, NULLIF($2,'')::uuid, NULLIF($3,''), NULLIF($4,''))
		 RETURNING id::text`,
		groupType, parentID, parentType, resourceRef).Scan(&id)
	if err != nil {
		return "", fmt.Errorf("create %q group: %w", groupType, err)
	}
	return id, nil
}

// GroupByResourceRef resolves a group by its API addressing key (type,
// resource_ref) — the route layer's (persona, resource-id). Returns the internal
...
func (st *PermissionGroupStore) GroupByResourceRef(ctx context.Context, groupType, resourceRef string) (string, error) {
	...
	err := st.q.QueryRow(ctx,
		`SELECT id::text FROM profiles.permission_groups
		 WHERE type = $1 AND resource_ref = $2 AND deleted_at IS NULL`,
		groupType, resourceRef).Scan(&id)
...
func (st *PermissionGroupStore) RootGroupID(ctx context.Context) (string, error) {
	var id string
	err := st.q.QueryRow(ctx,
		`SELECT id::text FROM profiles.permission_groups WHERE type = 'root' AND deleted_at IS NULL`).Scan(&id)
	if errors.Is(err, pgx.ErrNoRows) {
		return "", ErrGroupNotFound
	}
	return id, err
...
func (st *PermissionGroupStore) WalkAssignments(ctx context.Context, groupID, subjectID, subjectKind string) ([]GroupAssignment, error) {
	rows, err := st.q.Query(ctx,
		`WITH RECURSIVE chain AS (
			SELECT id, type, parent_id FROM profiles.permission_groups
			WHERE id = $1::uuid AND deleted_at IS NULL
			UNION ALL
			SELECT p.id, p.type, p.parent_id FROM profiles.permission_groups p
			JOIN chain c ON p.id = c.parent_id WHERE p.deleted_at IS NULL
		)
		SELECT c.id::text, c.type, a.role
		FROM chain c
	...
	for _, gid := range order {
		...
		out = append(out, GroupAssignment{GroupType: a.typ, GroupID: gid, Roles: a.roles})
	}
	return out, nil
}
