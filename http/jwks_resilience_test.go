...
func TestVerifierJWKSFetchResilience(t *testing.T) {
	...
	tok, err := MintDelegatedAccessToken(context.Background(), signer, DelegatedAccessParams{
		Issuer: iss, Audiences: aud, DelegatedSubject: "u1", Tenant: "t1", TenantID: "0190dead-beef-7000-8000-000000000001", TTL: time.Minute,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestVerifierRefetchesOnVerifyFailure(t *testing.T) {
	...
	t2, _ := MintDelegatedAccessToken(context.Background(), newSigner, DelegatedAccessParams{
		Issuer: iss, Audiences: aud, DelegatedSubject: "u2", Tenant: "t1", TenantID: "0190dead-beef-7000-8000-000000000001", TTL: time.Minute,
	})
