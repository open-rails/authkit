...
func seedPerfData(t *testing.T, ctx context.Context, pool copyExecDB, rootID string, scale int) {
	...
	// One active session per user, each with one demoted hash in history so the
	// historical-hash probe walks history PK -> refresh_sessions_family_active.
	...
	copyHistory := pgx.CopyFromSlice(scale, func(i int) ([]any, error) {
		return []any{prevHash(i), perfFamilyID(i)}, nil
	})
	if n, err := pool.CopyFrom(ctx, pgx.Identifier{"profiles", "refresh_token_history"},
		[]string{"token_hash", "family_id"}, copyHistory); err != nil || int(n) != scale {
		t.Fatalf("copy token history n=%d err=%v", n, err)
	}
	...
	for _, table := range []string{
		"profiles.users", "profiles.refresh_sessions", "profiles.refresh_token_history",
		"profiles.group_user_roles", "profiles.user_providers",
	} {
