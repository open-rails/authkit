...
SELECT id, email, username, phone_number FROM profiles.users WHERE id = sqlc.arg(user_id)::uuid
...
FROM profiles.account_erasure_obligations o, unnest(sqlc.arg(issuers)::text[]) AS issuer
WHERE o.user_id = sqlc.arg(user_id)::uuid
ON CONFLICT (user_id, issuer) DO NOTHING;

-- name: ErasureObligationLock :one
SELECT user_id FROM profiles.account_erasure_obligations WHERE user_id = sqlc.arg(user_id)::uuid FOR UPDATE;
