...
func TestLastOwnerGuard_DB(t *testing.T) {
	...
	if err := svc.RemoveGroupSubjectAs(ctx, owner, RootPersona, "", owner, SubjectKindUser); !errors.Is(err, ErrCannotRemoveLastAdminRole) {
		t.Fatalf("remove sole owner: got %v, want ErrCannotRemoveLastAdminRole", err)
	}
	ok, err := svc.Can(ctx, owner, SubjectKindUser, RootPersona, "", PermRootUsersBan)
	if err != nil || !ok {
