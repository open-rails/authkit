...
const (
	// RootPersona is the single built-in permission-group persona (the former
...
// PersonaDef declares one permission-group persona (a persona). persona ≡ type ≡
// the first permission segment. `Name == RootPersona` is the parentless singleton.
...
// RootPersona, parentless); every type has an `owner` role == `<type>:*`; every
...
// normalizePersona validates a declared type and injects the seeded owner/member
...
// validateRoot enforces exactly one parentless type, named RootPersona.
func (s *GroupSchema) validateRoot() (string, error) {
	...
	switch len(roots) {
	case 0:
		return "", fmt.Errorf("no root type declared (exactly one parentless type, named %q, is required)", RootPersona)
	case 1:
		if roots[0] != RootPersona {
			return "", fmt.Errorf("the parentless type must be named %q, got %q", RootPersona, roots[0])
		}
		return roots[0], nil
	default:
		sort.Strings(roots)
	...
	for name, t := range s.types {
		...
		for _, p := range t.AllowedParents {
			if _, ok := s.types[p]; !ok {
				return fmt.Errorf("group persona %q: allowed parent %q is not a declared type", name, p)
			}
			if seen[p] {
...
// proposed (childType, parentPersona) edge. root is parentless; every non-root
...
func (s *GroupSchema) ValidateParent(childType, parentPersona string) error {
	ct, ok := s.types[childType]
	if !ok {
		return fmt.Errorf("unknown group persona %q", childType)
	}
	if childType == RootPersona {
		if parentPersona != "" {
			return fmt.Errorf("the root group is parentless; got parent type %q", parentPersona)
		}
		return nil
	}
	if parentPersona == "" {
	...
	return fmt.Errorf("a %q group must have a parent of type %v, got %q", childType, ct.AllowedParents, parentPersona)
}
