...
func TestCan_WalkUpAndNamespacePurity(t *testing.T) {
	s := tensorhubSchema(t)
	orgOwner := []GroupAssignment{{Persona: "org", GroupID: "g_org", Roles: []string{"owner"}}}
	...
	repoColl := []GroupAssignment{{Persona: "repo", GroupID: "g_repo", Roles: []string{"writer"}}}
...
func TestResolveGrants_CustomRoles(t *testing.T) {
	...
	orgCustom := []GroupAssignment{{Persona: "org", GroupID: "g_org", Roles: []string{"auditor"}}}
	...
	repoCustom := []GroupAssignment{{Persona: "repo", GroupID: "g_repo", Roles: []string{"auditor"}}}
