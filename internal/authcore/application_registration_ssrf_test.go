...
// #288/14: outside dev the registration fetch dials through the netguard
// transport, which resolves the domain itself and refuses any private/reserved
// answer — so a public name that resolves to loopback, RFC1918 or the cloud
...
func TestRegisterApplicationFromDomainGuardsDialOutsideDev(t *testing.T) {
	...
	for name, ip := range map[string]string{"loopback": "127.0.0.1", "rfc1918": "10.0.0.5", "metadata": "169.254.169.254"} {
		t.Run(name, func(t *testing.T) {
			svc.appHTTPClient = newApplicationsHTTPClient(false, resolverFunc(func(_ context.Context, host string) ([]net.IPAddr, error) {
