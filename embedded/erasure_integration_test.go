...
func TestErasureHandoffAcrossSites(t *testing.T) {
	...
	site := func(issuer string, account ...string) *engine {
	...
	listAll := func(c *engine, issuer string, limit int) []authkit.ErasureObligation {
		t.Helper()
	...
	purgeable := func(c *engine) map[string]bool {
		t.Helper()
