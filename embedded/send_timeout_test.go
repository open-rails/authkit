...
func TestVerificationSendTimeout_DefaultAndConfigurable(t *testing.T) {
	if got := (&Client{}).verificationSendTimeout(); got != 15*time.Second {
		t.Fatalf("default timeout = %v, want 15s", got)
	}
	s := &Client{cfg: Config{Registration: RegistrationConfig{VerificationSendTimeout: 2 * time.Second}}}
	if got := s.verificationSendTimeout(); got != 2*time.Second {
		t.Fatalf("configured timeout = %v, want 2s", got)
	}
	// A nil Client must not panic and must fall back to the default.
	if got := (*Client)(nil).verificationSendTimeout(); got != 15*time.Second {
...
func TestWithSendTimeout_BoundsBlockingSend(t *testing.T) {
	s := &Client{cfg: Config{Registration: RegistrationConfig{VerificationSendTimeout: 50 * time.Millisecond}}}
	start := time.Now()
...
func TestWithSendTimeout_PassThrough(t *testing.T) {
	s := &Client{cfg: Config{Registration: RegistrationConfig{VerificationSendTimeout: time.Second}}}
