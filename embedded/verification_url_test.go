...
func TestVerificationLinkURLs_UseConfiguredFrontendPaths(t *testing.T) {
	svc := mustNewWithKeys(t, Config{Token: TokenConfig{Issuer: "https://issuer.example"}, Frontend: FrontendConfig{BaseURL: "https://app.example", VerifyPath: "/verify-registration", PasswordResetPath: "/reset-password"}}, Keyset{})
...
func TestVerificationLinkURLs_Defaults(t *testing.T) {
	svc := mustNewWithKeys(t, Config{Token: TokenConfig{Issuer: "https://issuer.example"}, Frontend: FrontendConfig{BaseURL: "https://app.example"}}, Keyset{})
