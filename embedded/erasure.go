...
// "" on the last one.
func (s *Client) ListErasureObligations(ctx context.Context, site, after string, limit int) ([]authkit.ErasureObligation, string, error) {
	if s.pg == nil {
		return nil, "", nil
	}
...
// miss the close.
func (s *Client) AcknowledgeErasure(ctx context.Context, site, userID string) error {
	if s.pg == nil {
		return nil
	}
	site = strings.TrimSpace(site)
...
func (s *Client) ErasureBacklog(ctx context.Context) ([]authkit.ErasureSiteBacklog, error) {
	if s.pg == nil {
		return nil, nil
	}
...
func (s *Client) raiseErasureObligationTx(ctx context.Context, q *db.Queries, userID string) error {
