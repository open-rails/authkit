...
func TestProviderLinkSerializesWithRevocation(t *testing.T) {
	...
	_, err = svc.qtx(revocation).SessionsRevokeAll(ctx, db.SessionsRevokeAllParams{UserID: u.ID, Issuer: svc.cfg.Token.Issuer})
	require.NoError(t, err)
	...
	require.NoError(t, svc.RevokeAllSessions(ctx, u.ID, nil))
	sessions, err = svc.ListUserSessions(ctx, u.ID)
	require.NoError(t, err)
	require.Empty(t, sessions)
}
