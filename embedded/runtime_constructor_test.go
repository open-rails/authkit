...
func TestRuntimeConstructorOwnsTopologyWithoutRestoringRoles(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, client.OperatorAssignGroupRole(t.Context(), authkit.RootGroup(), authkit.UserSubject(user.ID), "editor"))
	require.NoError(t, client.OperatorUnassignGroupRole(t.Context(), authkit.RootGroup(), authkit.UserSubject(user.ID), "editor"))
	first.Close()
...
func TestRuntimeConstructorWithoutRBACPreservesSharedTopology(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, owner.Client().OperatorAssignGroupRole(t.Context(), authkit.RootGroup(), authkit.UserSubject(user.ID), "editor"))
