...
func TestExternalLoginOutcomes_DB(t *testing.T) {
	...
	t.Run("fresh identity: account created, welcome sent, session issued; second login reuses it", func(t *testing.T) {
		...
		require.Equal(t, LoginSessionIssued, out.Kind)
	...
	t.Run("existing local email refuses a silent link; the authenticated link flow binds it", func(t *testing.T) {
		...
		sid, _, _, err := h.svc.IssueRefreshSession(ctx, victim.ID, "link", nil)
		require.NoError(t, err)
		fresh, err := h.svc.SessionFreshness(ctx, victim.ID, sid, time.Now())
		require.NoError(t, err)
		out, err := h.svc.CompleteExternalLogin(ctx, ExternalLoginInput{Identity: id, Link: &ExternalLinkAuthorization{UserID: victim.ID, SessionID: sid, AuthenticatedAt: fresh.LastAuthenticatedAt}, Event: "oidc_login"})
