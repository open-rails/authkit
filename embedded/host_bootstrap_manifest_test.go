...
`), 0600))
	...
	dry, err := svc.Client().OperatorApplyBootstrapManifest(ctx, manifest, BootstrapReconcileOptions{DryRun: true})
	require.NoError(t, err)
	...
	first, err := svc.Client().OperatorApplyBootstrapManifest(ctx, manifest, BootstrapReconcileOptions{StartupOnly: true, Name: "first"})
	require.NoError(t, err)
	...
	for _, name := range []string{"first", "second", "second"} {
		result, err := svc.Client().OperatorApplyBootstrapManifest(ctx, requested, BootstrapReconcileOptions{StartupOnly: true, Name: name})
		require.NoError(t, err)
	...
	result, err := svc.Client().OperatorApplyBootstrapManifest(ctx, manifest, BootstrapReconcileOptions{})
	require.NoError(t, err)
	...
	_, err = svc.Client().OperatorApplyBootstrapManifest(ctx, recovery, BootstrapReconcileOptions{})
	require.NoError(t, err)
	...
	result, err = svc.Client().OperatorApplyBootstrapManifest(ctx, BootstrapManifest{RemoteApplications: []BootstrapManifestRemoteApplication{app}}, BootstrapReconcileOptions{})
	require.NoError(t, err)
