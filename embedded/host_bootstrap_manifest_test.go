...
`), 0600))
	...
	dry, err := svc.Client().AdminApplyBootstrapManifest(ctx, manifest, BootstrapReconcileOptions{DryRun: true})
	require.NoError(t, err)
	...
	first, err := svc.Client().AdminApplyBootstrapManifest(ctx, manifest, BootstrapReconcileOptions{StartupOnly: true, Name: "first"})
	require.NoError(t, err)
	...
	for _, name := range []string{"first", "second", "second"} {
		result, err := svc.Client().AdminApplyBootstrapManifest(ctx, requested, BootstrapReconcileOptions{StartupOnly: true, Name: name})
		require.NoError(t, err)
	...
	result, err := svc.Client().AdminApplyBootstrapManifest(ctx, manifest, BootstrapReconcileOptions{})
	require.NoError(t, err)
	...
	_, err = svc.Client().AdminApplyBootstrapManifest(ctx, recovery, BootstrapReconcileOptions{})
	require.NoError(t, err)
	...
	result, err = svc.Client().AdminApplyBootstrapManifest(ctx, BootstrapManifest{RemoteApplications: []BootstrapManifestRemoteApplication{app}}, BootstrapReconcileOptions{})
	require.NoError(t, err)
