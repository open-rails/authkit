...
func TestNamingEnvironmentAndEmbeddedPolicyAgree(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			client, err := embedded.New(embedded.Config{Naming: c.naming, Keys: embedded.KeysConfig{VerifyOnly: true}, Token: embedded.TokenConfig{Issuer: "https://auth.test", IssuedAudiences: []string{"test"}}}, nil)
			if err != nil {
				t.Fatal(err)
			}
