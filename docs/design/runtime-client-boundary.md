...
After the host applies migrations, database-backed construction atomically installs declared group containment and the root singleton under the authority lock. It never assigns user roles or restores revoked authority. Explicit operator commands use Client.AdminAssignGroupRole and Client.AdminUnassignGroupRole; actor-bearing requests use the existing checked As operations. Both preserve subject MFA and final-owner invariants.
