...
After the host applies migrations, database-backed construction atomically installs explicitly declared group containment and the root singleton under the authority lock. Omitted or empty RBAC leaves existing shared topology intact. Construction never assigns user roles or restores revoked authority. Explicit operator commands use Client.OperatorAssignGroupRole and Client.OperatorUnassignGroupRole; actor-bearing requests use the existing checked As operations. Both preserve subject MFA and final-owner invariants.
