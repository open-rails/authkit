...
| `TestBrowserDelegationWorkflow` | Real HTTP login, opt-in browser mint and TLS resource access over memory/Redis; authority clamps, wrong key/method/path/token, replay concurrency, proxy target rewriting, native default refusal and replay backend outage/recovery. |
