...
`/groups/:group/members` (add/remove/list) · `/groups/:group/members/:user/roles` (assign/unassign) · `/groups/:group/roles[/:role]` (define/list/delete, incl. custom roles) · `/groups/:group/api-keys` (mint/list/revoke) · `/groups/:group/permissions` · `/me/groups` · invites. Plus authkit's existing AUTH/IDENTITY HTTP (login/register/token/refresh/`/me`/sessions/2FA/OIDC/JWKS) and the intrinsic `/admin/*` on authkit-owned objects.
**HOST owns (calls core):** RESOURCE LIFECYCLE — create/delete the org/repo/merchant *record* + its paired group (touches host tables + side effects: seed billing, notify, the org-slug lifecycle gated by app-declared `root:orgs:*`). The `org`-NAMED routes are DROPPED (org isn't authkit's); the generic `/groups/:group/*` covers their management ground.
...
- [ ] HTTP surface (DECIDED — maximize authkit, keep hosts thin): ship GENERIC type-parameterized group-management routes `/groups/:group/{members, members/:user/roles, roles[/:role], api-keys, permissions}` + `/me/groups` + invites (generalize today's `/orgs/:org/*`), gated by the per-type `<type>:members:manage`/`:roles:manage`/`:api-keys:manage`/`:read`. Keep authkit's auth/identity HTTP + intrinsic `/admin/*`. HOST owns resource-lifecycle + domain-side-effect routes (create/delete the resource record + its paired group via core; org-slug lifecycle gated by app-declared `root:orgs:*`). Drop the `org`-named routes.
