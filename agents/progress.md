...
- **Built-in perms (authkit-provisioned in EVERY type's catalog — the group-self-management set):** `<type>:members:manage`, `<type>:roles:manage`, `<type>:api-keys:manage`, `<type>:read`. They gate the generic group-management routes below; the app adds its DOMAIN perms alongside (all `<type>:`-namespaced). `root` also ships the intrinsic identity perms (`root:users:*`, `root:groups:*`, `root:sessions:revoke`, …).

## HTTP surface — who ships which routes (DECIDED: maximize authkit, keep hosts thin)
**authkit ships, host mounts** — type-GENERIC, gated by the per-type management perms above (= today's `/orgs/:org/*` set, generalized from "org" to any type):
`/permission-groups/:group/members` (add/remove/list) · `/permission-groups/:group/members/:user/roles` (assign/unassign) · `/permission-groups/:group/roles[/:role]` (define/list/delete, incl. custom roles) · `/permission-groups/:group/api-keys` (mint/list/revoke) · `/permission-groups/:group/permissions` · `/me/permission-groups` · invites. Plus authkit's existing AUTH/IDENTITY HTTP (login/register/token/refresh/`/me`/sessions/2FA/OIDC/JWKS) and the intrinsic `/admin/*` on authkit-owned objects.
**HOST owns (calls core):** RESOURCE LIFECYCLE — create/delete the org/repo/merchant *record* + its paired group (touches host tables + side effects: seed billing, notify, the org-slug lifecycle gated by app-declared `root:orgs:*`). The `org`-NAMED routes are DROPPED (org isn't authkit's); the generic `/permission-groups/:group/*` covers their management ground.
...
- [ ] HTTP surface (DECIDED — maximize authkit, keep hosts thin): ship GENERIC type-parameterized group-management routes `/permission-groups/:group/{members, members/:user/roles, roles[/:role], api-keys, permissions}` + `/me/permission-groups` + invites (generalize today's `/orgs/:org/*`), gated by the per-type `<type>:members:manage`/`:roles:manage`/`:api-keys:manage`/`:read`. Keep authkit's auth/identity HTTP + intrinsic `/admin/*`. HOST owns resource-lifecycle + domain-side-effect routes (create/delete the resource record + its paired group via core; org-slug lifecycle gated by app-declared `root:orgs:*`). Drop the `org`-named routes.
...
6. **OPEN — `root` is NOT always a per-deployment singleton.** doujins + hentai0 (#416 / #176) share ONE AuthKit DB (shared user IDENTITY) but need SEPARATE moderation authority — a doujins admin must NOT moderate hentai0. So a single global `root` is wrong for shared-identity multi-app deployments. Decide: support MULTIPLE named roots (one tree per app over shared users) OR per-app root scoping. (hentai0 #176 leans separate roots; flat single-app deployments like openrails/tensorhub keep one `root`.)
7. **CLARIFY — `root` is the OPERATOR layer, not strictly "moderation."** Its catalog includes staff OPERATIONAL/visibility privileges, not just moderation ACTIONS — e.g. doujins `root:premium:bypass`, `root:ratelimit:bypass`, `root:content:view-restricted` (#416). reach ≠ capability still holds (they're `root:`-namespaced, can't impersonate or reach `merchant:`/`org:`/user-self). So read "root = moderation-only" everywhere above as "root = operator perms (moderation + operational/visibility privileges)."
