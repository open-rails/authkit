...
- **Mint authorization is native + permission-based:** minting requires `org:tokens:manage`, and authkit validates the requested permissions against the org's effective catalog — each must be a defined permission the caller holds (no escalation; `403 permission_grant_denied`/`400 unknown_permission`), with reserved `org:*` perms and wildcards barred from OATs. Permissions are frozen at mint time. An OAT can never mint/list/revoke OATs (no user). See "Org RBAC" below for the catalog + role→permission model.
