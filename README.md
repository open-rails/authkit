...
- **Mint authorization is native + permission-based:** minting requires `org:api_keys:manage`, and authkit validates the requested permissions against the org's effective permission set — each must be a defined permission the caller holds (no escalation; `403 permission_grant_denied`/`400 unknown_permission`), with the reserved write/mint perms (`org:roles:manage`, `org:members:manage`, `org:api_keys:manage`) and wildcards barred from API keys (read-only `org:read` is API-key-grantable). Permissions are frozen at mint time. An API key can never mint/list/revoke API keys (no user). See "Org RBAC" below for the permission + role→permission model.
