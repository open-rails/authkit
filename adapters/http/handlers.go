...
func (s *Service) APIHandler() http.Handler {
	...
	mux.Handle("POST /auth/admin/account/park", admin(http.HandlerFunc(s.handleAdminAccountParkPOST)))
	mux.Handle("POST /auth/admin/account/claim", admin(http.HandlerFunc(s.handleAdminAccountClaimPOST)))
	// Hard-cut legacy org park/claim naming.
	mux.Handle("POST /auth/admin/org/park", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		notFound(w, "not_found")
	}))
	mux.Handle("POST /auth/admin/org/claim", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		notFound(w, "not_found")
	}))
