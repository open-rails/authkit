...
// the Verifier's JWKS fetches and remote-application issuer registration POST.
// Without a timeout a slow or hostile endpoint can wedge a request goroutine indefinitely; when the
...
// does not inject its own via WithHTTPClient /
// WithRemoteApplicationIssuersHTTPClient.
